...
{
    [Cmdlet(iControlVerbs.Download, iControlNouns.File)]
    public class DownloadFile : iControlPSCmdlet
    {

        #region Parameters

        private string _local_file = null;
        [Parameter(Position = 0, Mandatory=true, HelpMessage = "The name of the remote file to download")]
        ...
        [Parameter(Position = 1, Mandatory = true, HelpMessage = "The local file to download the remote file into")]
        [ValidateNotNullOrEmpty]
        ...
        protected override void ProcessRecord()
        {
            if (isInitialized())
            {
                try
                {
                    ...
                    FileStream fs = new FileStream(_local_file, fm);
                    BinaryWriter w = new BinaryWriter(fs);

                    while (bContinue)
                    {
                        ctx = GetiControl().SystemConfigSync.download_file(_remote_file, chunk_size, ref file_offset);

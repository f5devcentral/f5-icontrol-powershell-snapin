...
function Get-PoolMemberStats([string] $pool_name, [string] $member)
{
	$member_def = New-Object -TypeName iControl.CommonIPPortDefinition;
	$member_def.address,$member_def.port = $member.Split(':');
	
	[iControl.CommonIPPortDefinition[]] $ipdefs = @($member_def);
	[iControl.CommonIPPortDefinition[][]] $ipdefslists = @(,$ipdefs)
...
{
	Get-PoolMemberStats $g_pool $g_member;
}
